...
export async function createCatalogServer(options: CreateServerOptions) {
  ...
  app.get('/health', async () => ({
    ok: true,
    service: 'msfs-compat-catalog-api',
  ...
  app.post<{ Body: FingerprintRequest }>('/v1/aircraft/fingerprint', async (req, reply) => {
    ...
    const result = await store.registerFingerprint(body);
  ...
  app.get<{
    ...
    const resolved = await store.resolveProfile(fingerprint, simVersion, channel ?? 'stable');
    if (!resolved) {
  ...
  app.get<{
    ...
    const doc = await store.getDocument(req.params.profileKey, semver);
    if (!doc) {
