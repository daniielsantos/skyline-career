...
**False freeze after Resume / cleanup (2026-09-24):** sintoma = chip preso após Resume. Premissas erradas (0.3.304–0.3.305): “IS PAUSED sticky” + overrides por posição/`ABSOLUTE TIME`/slew falso. Causa real = empacotar `IS SLEW ACTIVE` / `ABSOLUTE TIME` no FLOAT64 **desalinhou** o snapshot (`simAbsSec:1` = sim rate; slew sticky com GS 400). Fix = Host só lê `IS PAUSED` (slew live = false); Watch = `paused || slew` simples; fase fina atualiza mesmo com clock frozen; log `playback freeze` em `watch-debug.log`. Removidos AbsoluteTime DTO, `inspectSimPlaybackFreeze`, overrides sticky.
