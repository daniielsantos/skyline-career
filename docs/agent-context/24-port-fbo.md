...
**Dev Claim port picker (2026-09-25):** Settings → Developer → Ports = select de `CAREER_PORTS` + Claim (não mais só Santos). API `GET /api/debug/port-catalog` + `POST /api/debug/claim-port` com `portId` obrigatório/válido.

**Scout Haul intl/long only (2026-09-25):** sintoma = Port FBO Miami Scout (All/Haul) só SY*/SK*/TFFF/… ≥~1275 nm; nenhum K* US ~300–500 nm. Causa = **não** há filtro intl — Haul = short-fill (dest fill ≤40%) ≤1800 nm, top **8** por `pay − nm − fill`. Nearby US costuma fill &gt;40% (gate) ou score perde pra haul longo (pay↑). Demand = outro board (corridor P1 **500** nm). Não é bug de país. **Por que US “cheio”:** densify + NPC/lots mantêm majors/regionais mid–high fill; Caribe/SA caem no band ≤40% (Fill 7–37% no board). Haul = restock terminal vazio, não feeder doméstico — curto US = Demand/Freights. Não retunar `CARGO_FLOW_BALANCE` só por Scout. **Score:** `payUsd` (quote freight, sobe com nm/intl) − `min(nm,800)×0.2` − `fill×800` — penalidade de distância teto **$160**; pay ~$17k domina → prioriza longo. **Demand P3 open:** Accept sem teto nm; spawn catchment mundial, mas desk ainda **~6–7** orders (`DEMAND_ORDERS_PER_PORT_*` + hub/country caps) — Scout Demand top 8 por `pay − min(nm,800)×0.25` (mesmo viés pay/longo).

**Design opinion — long-haul bias both boards (2026-09-25):** Haul lean longo = **aceitável** na fantasia (restock terminal vazio; curto US = Demand/Freights). Demand lean longo no P3 = **não** tão bom: P1→P3 deveria abrir *opção* worldwide, não esvaziar o desk regional — shelf ~6–7 + score pay-dominado → board colapsa em intercontinental. Correção desejável (se/quando): band quotas ou nm penalty real / `$/hr`, **sem** Dry/`CARGO_FLOW_BALANCE`. Não shipado — só opinião.

**Demand desk band mix (2026-09-25):** sintoma = P3 open catchment + Scout `pay − min(nm,800)×0.25` → desk/Scout só long haul. Causa = spawn linear mundial sem anéis; multa nm irrelevante vs pay. Fix = spawn por bandas exclusivas (`demandSpawnBandTargets`: P1 all ≤500; P2 ~½/½; P3 **2/2/resto** ≤500 / (500,1800] / >1800) sem roubar slots entre anéis; Scout Demand `payUsd − distanceNm×2` (sem teto). Accept P3 continua open. Haul/Dry inalterados.
