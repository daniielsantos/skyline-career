    ...
    >
      ...
      {showLoadPanel && loadPath !== 'inject' ? (
        <div className="dispatch-step-card" aria-live="polite">
          ...
          <p
            title={
              loadPath === 'efb'
                ? 'kg mode on the EFB often writes the wrong mass into the sim and Loaded vs Due fails'
                : undefined
            }
          >
            {loadPath === 'efb'
              ? `Import SimBrief on the ${assignedAircraft} EFB/FMC · weight units LB`
      ...
      {showPreflight && mission.lastPreflightCheck
        ? (() => {
            ...
            // Keep the EN ROUTE live-tile layout for resume prep (don't switch to
            // the denser preflight-load-grid — that looked "shrunk").
            const enRouteTiles = enRoute;
            // Resume prep still needs honest Due match marks (not soft cruise ·).
            const softLiveMarks = enRoute && !resumePrep;
            ...
            const enRouteCruise =
              ofpBriefing?.cruiseAltitudeFt !== undefined
                ? ofpBriefing.cruiseAltitudeFt >= 18_000
                  ? `FL${String(
                      Math.round(ofpBriefing.cruiseAltitudeFt / 100),
                    ).padStart(3, '0')}`
                  : `${Math.round(ofpBriefing.cruiseAltitudeFt).toLocaleString('en-US')} FT`
                : undefined;
