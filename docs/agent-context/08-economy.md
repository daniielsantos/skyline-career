...
- **Charter / schema v9 (2026-09-07):** passenger Terminal pools (`charterHubs.waitingPax` / `attractPax`) drive continuous offer formation like commodity surplus→shortage — **not** a daily dump. Soft OD heat stays in `charter_demand`; offers form each tick (quota **~4**, warm **~12** if board &lt;120), TTL **8–24 h** staggered, board target **~hubs/3** (floor **120**, cap **600**). Domestic picks **country first** (pool-weighted, distance-viable ODs) with a **per-country live share cap** + starved-top coverage so BR/US are not left empty while a mid-size country monopolizes; expire returns pax to pools; settle consumes them. Isolated from freight stock/NPC/Cargo Ops. Baggage = 18 kg/pax capped 180 kg. **Eligible classes:** `light_ga` / `light_turboprop` / `light_jet` (shared 1–12 pax board; Fit gates seats/range/passenger config — Narrow joins later when group size grows). Offer **tiers** `standard|premium|executive` (product umbrella is **Charter**, not Executive). Market/Terminal/Base → fixed Manifest; SimBrief exact `pax=N` + baggage-only cargo; `charter_payout`. SQLite: `charter_hubs` + `charter_demand` / `charter_offers` + mission columns. Legacy OD pressure migrates into hub pools on ensure. **Terminal Inventory** shows one passenger row in the commodity table (stock = waiting; Flow = attract + live out/in) via `GET /api/airport` → `charter` — never mixes into commodity WH math.
...
- **Market internacional dinâmico (2026-09-07):** fretes domésticos continuam por país; intl usa grafo diário deterministicamente selecionado por pressão de estoque e gateways (`career-international-lanes.ts`), sem tabela fixa de países/aeroportos. Bounds: **96–480** lanes, alvo **2.25/país**, piso **2/país**, cap **6/país** e **2/par**, long-haul ≥20%; ODs com lote ativo atravessam a virada como carry-over. Fatia intl do board segue **12%**, preservando doméstico. Pulse mostra lanes/countries/carry-over/caps.
...
- **Freight boards (2026-09-07):** Freights split into **Aircraft needed** (sua aeronave · lot pay) e **Crew needed** (avião NPC · pilot fee); filtro API `?crew=aircraft|crew`. Route inclui **Domestic · pilot country** (`lane=pilot-domestic`): exige OD doméstico e origem no país do `pilotIcao` atual (ex.: piloto em SBKP → origem BR). Terminal Contracts espelha com chips Aircraft/Crew. Hangar vazio abre em Crew.
