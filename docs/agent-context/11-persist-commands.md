...
**Sticky IS PAUSED after Resume freezes chip (2026-09-24):** sintoma = após ESC→Resume o chip fica preso (ex. `20m/59m · 33%`) mesmo voando. Causa = MSFS 2024 pode deixar `IS PAUSED` true com o avião já em movimento; Watch congelava o clock para sempre. Fix 0.3.304 = posição ≥~0.015 nm — **insufficiente** quando lat/lon soft-hold copia o sample anterior. Fix 0.3.305 = `ABSOLUTE TIME` no snapshot — **quebrava alinhamento** do FLOAT64 block (`simAbsSec:1` = sim rate; `slew:true` sticky com GS 400kt). Fix = Host só empacota `IS PAUSED` no snapshot (sem SLEW/ABSOLUTE TIME); Watch ignora slew falso se GS≥30 ou posição move; fase fina atualiza mesmo com clock frozen; log `playback freeze` em `watch-debug.log`.
