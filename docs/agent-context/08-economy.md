...
- **Tick bench:** `benchEconomyTicks` / CLI `career tick --bench`. Seed global **1137** hubs / **3839** NPCs. Fase 2b (rng por partição×commodity + fita NPC): tick 1 ~**10s**, regime ~**2s**/tick, +1 day ~**3 min** in-memory. Produção/fuel/eventos ainda na fita `seed:tN`; lots/NPC não replay vs Fase 2. Sem capar frota / Dry.
