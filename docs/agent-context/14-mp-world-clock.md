...
- **World CI/CD amd64-only (2026-09-17):** `World deploy` builda só `linux/amd64` (Pi / arm64 + QEMU removidos; sem job `deploy-staging`). CI verde em `main` publica `sha-<commit>` + `main`. Release espera um sibling `workflow_run` com `conclusion=success` (ignora `cancelled`), retaga o digest como `v*` e promove à VPS após Environment `production`. Setup: `deploy/README.md`.
