...
- **Tick bench / event loop:** `tickEconomy` continua síncrono (testes, catch-up no load). `tickEconomyCooperative` cede entre países no formLots (`setImmediate`); `POST /api/tick` usa isso. Cadeado world+company permanece no write inteiro. Sem workers / Dry.
...
- **Demand Board:** NPC buy-orders quando stock do hub está baixo (cap **192–640** open, **~6 por país**; wanted até **12 t** general / **8 t** electronics). UI default **My warehouses** (dest só domestic/allowlist a partir da WH). **Hold** pledges WH stock + decrements world `remainingKg` (TTL T1/T2/T3 = 48/72/96 ticks); **Fly now / Dispatch** = missão WH→dest. Settle = payout + fill terminal + credit `lifetimeShippedKg`. **Edit cargo** restores/withdraws WH + demand remaining. **Intl (port-fed):** cross-border só se par de país na allowlist **e** origem WH em pickup hub; pay × **1.28**; Market `CAREER_INTERNATIONAL_LANES` intactas.
- **Ports loop UX (slice 1):** `derivePortsLoopStep` — com WH mas stock 0 + `inboundTransfers` → `wait_inbound` (não `buy_port`); com stock sem match → `wait_demand` + `openDemandCount`. Banner quando aba ≠ passo; hint na Demand; empty board menciona tick (~15 min) + CTA All destinations.
