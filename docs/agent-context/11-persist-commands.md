...
**Sticky IS PAUSED after Resume freezes chip (2026-09-24):** sintoma = após ESC→Resume o chip fica preso (ex. `20m/59m · 33%`) mesmo voando. Causa = MSFS 2024 pode deixar `IS PAUSED` true com o avião já em movimento; Watch congelava o clock para sempre. Fix 0.3.304 = posição ≥~0.015 nm — **insufficiente** quando lat/lon soft-hold copia o sample anterior. Fix = Host lê `ABSOLUTE TIME`; `inspectSimPlaybackFreeze` trata pause sticky como live se o tempo do sim avança ≥0.2s (ou posição fresh move); Watch loga `playback freeze` em `watch-debug.log` (transition + heartbeat 10s).
