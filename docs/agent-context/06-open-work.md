...
Atualizado 2026-09-20: **PG schema v29 misc leftovers** — kill switch em `force_client_update` / `min_client_version`; `flow_stats` + tabelas `international_lanes` / `port_inbound_ships` / `tour_lot_soft_holds` / `regional_recovery` / `presence_events`; `misc_json` esvaziado. Ops SQL: [`14-mp-world-clock.md`](./14-mp-world-clock.md).

Atualizado 2026-09-20: **Client update kill switch** — rare `force_client_update` + `min_client_version` (v29); health + 426 on Prepare/Accept/Fly now across Freights, Charter, Ports Demand/Bridge/Haul/shuttle, Base Dispatcher; Hold at WH stays open. Ops SQL + surface table: [`14-mp-world-clock.md`](./14-mp-world-clock.md).
...
Atualizado 2026-09-17: **Freights / Charter nav** — Charter sai dos chips de Freights para sidebar `/charter`; Freights = Your aircraft | Operator aircraft. Spec: [`08-economy.md`](./08-economy.md).
...
Atualizado 2026-09-14: **MP PG world-ops + dealer pool** — `career-store-pg-world.ts` has v5 ops (`npcs` / fuel / demand / ports / flights / events) + `aircraft_instances`; hydrate/persist/strip wired; load backfills empty tables from RAM; BIGINT ms truncated. Spec: [`14-mp-world-clock.md`](./14-mp-world-clock.md).
