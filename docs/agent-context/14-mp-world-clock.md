...
- **Diag 2026-09-25 — AuthGate “Select a career profile first” after PG truncate:** sintoma = login MP pós-wipe mostrava copy de SP. Causa = `/api/auth/login` com `store == null` reusava `needs_profile`. Fix = em `CAREER_WORLD_FIXED`, auth sem store → **503** `world_store_opening` / `world_store_unavailable` (“World is not ready…”). Conta apagada com store open continua **401** `invalid login or password`.
