...
- **Compose overlays (2026-09-14):** base `docker-compose.yml` = Postgres **sem** publish. **Lab** `docker-compose.lab.yml` → `127.0.0.1:5432` + Adminer `:8081` + world `:8787`. **Prod** `docker-compose.prod.yml` → world `127.0.0.1:8787` + Postgres **`127.0.0.1:5432`** (SSH tunnel; nunca `0.0.0.0`) — sem Adminer. `npm run career:stack:world` = lab; `--prod` = VPS-safe; `--prod --tls` = + **Caddy** (`deploy/Caddyfile`, profile `tls`, `CAREER_WORLD_HOST` + DNS + :80/:443 → Let's Encrypt; alias legado `SKYLINE_WORLD_HOST`). Desktop: `CAREER_WORLD_API_URL=https://<host>`.

- **Prod PG loopback (2026-09-25):** sintoma = DBeaver SSH ok mas EOF em `127.0.0.1:5432` após recreate. Causa = prod não publicava PG no host (só rede Docker; IP do container muda). Fix = `docker-compose.prod.yml` `postgres.ports: 127.0.0.1:5432:5432`. DBeaver Main continua Host `127.0.0.1` + aba SSH.
...
- **Sim local / world host (2026-09-14):** production split wired.
  ...
  - **Arrivals NPC progress bar rewinds (2026-09-24):** sintoma = barra Arrivals/Fleet avança e depois volta (CSS `transition: width` deixa óbvio). Causa = `% = (now−departed)/(arrives−departed)` com `displayNowMs` corrigido a cada poll de `/api/world/clock` e `/api/state`; o snap (RTT/offset) pode rebobinar `now` uns segundos → `%` cai 1pt e a barra anima para trás. ETA vs % entre rows **não** é bug (rotas com durações diferentes). Fix = (1) `coalesceDisplayNowMs` ignora rewind &lt;3s; (2) `holdMonotonicPct` high-water por flight+stamps em MovementBoard/FleetRoster (retime real de stamps reseta a chave).
...
- **Diag 2026-09-25 — update mid-deploy → ProfileGate “World” + Signed in:** sintoma = app update + world inacessível → erro de request → Select a profile (World / Last played) com “Signed in on this tab”, não AuthGate nem Listening. Causa = Bearer ainda válido (login **não** é o próximo passo); ProfileGate é o fallback SP / flicker `!worldFixed` no poll — MP deveria ficar em **WorldWaitingGate** até o host voltar. AuthGate só em **401** (sessão morta). Não forçar login com token vivo. **Fix:** `shouldKeepWorldWaitingOnUnfixedHealth` (playMode `mp` ou `sawWorldFixed`) — boot + poll **não** abrem ProfileGate no blip; SP explícito ainda sai pra picker. Play mode lido **antes** do 1º health no boot.



- **Diag 2026-09-25 — truncate + Product key sumiu / mundo morto:** truncate **não** altera `.env`. Live `/api/auth/status` já tinha `accessKeysRequired:true`, mas `/api/health` → `world_store_unavailable` porque prod tem `CAREER_WORLD_ALLOW_SEED=0` e o PG ficou vazio → open recusa reseed. Procedimento: `CAREER_WORLD_ALLOW_SEED=1` → recreate world-api → seed → voltar `=0` → recreate; remint access keys se a tabela foi truncada. UI: AuthGate refresha invite/product-key flags no `AUTH_REQUIRED` / sign-out (antes ficava sticky `false`).
