...
**MP inject `Authentication required` (2026-09-16; fixes v0.3.69/v0.3.70):** inject físico é local (desktop → SimBridge), mas `/api/load-ofp` lê missão/frota no VPS antes e salva Preflight/ballast no VPS depois; `/api/preflight` e `/api/watch/start` têm o mesmo split. v0.3.69 removeu o Bearer global race-prone via `WorldApiAuthScope`/`AsyncLocalStorage`, preservou `WorldApiError` upstream e abriu AuthGate em 401. Falha restante: `postLoadOfp` usava `fetch` cru e nunca enviava Bearer/company; v0.3.70 centraliza todo Career HTTP em `careerFetch` e migra também downloads PLN/GFP (outros dois bypasses encontrados). Audit: só assets de áudio usam `fetch` cru; `/load-ofp`, `/preflight`, `/watch/start` usam auth request-scoped; `/simbridge`, progress/cancel e watch status/stop são locais; Dispatch/OFP/boards vão por proxy; settle/depart encaminham auth diretamente.
...
- **Intent vs manualpayload split (2026-08-30):** freighter `manualpayload=` + EFB `pax=1` → SimBrief Payload = missão, partido em bag + ~221 lb pax (ATR RCTP→RCSS 3536/3757). `ofpFreightTowardMissionKg` com `missionCargoKg`: se Payload ≈ missão → conta **Payload** (PASS); se Payload << missão (Duke cut) → **bag**. Evita FAIL falso “MTOW limited” sem alerta no SimBrief.
...
- **Inject engines-off falso após cutoff (2026-09-20):** sintoma — Aircraft tile “Engines off”, toggle inject → tile vira “Engines running” + `Shut down engines before fuel inject`. Causa — (1) Watch aplicava `forceEnginesOffWhenParkedSpoolDead` e o inject/probe não; durante `isOfpLoadActive` a UI cai no `lastProbeSnapshot` do inject. (2) `inferEnginesRunning` deixava PPH residual ganhar de RPM=0 (Accu-Sim pós-cutoff). Fix — helper compartilhado no probe/inject + RPM morto antes do gate de flow.
...
| # | Gate | Pass |
|---|------|------|
| A1 | `writetest` / wizard discovery | Tanques + stations writable; soft re-probe; sem `UNRECOGNIZED_ID`; COUNT>16 avisado; discovery max **48** |
