...
**PHASE CLIMB + MISSION DISPATCHED (2026-09-25):** sintoma = footer `MSFS` + `PHASE CLIMB` / Aircraft Airborne, stepper preso em READY / DISPATCHED. Causa = (1) `evaluateMissionFlightTransition` chamava `isSimPlaybackFrozen(sample)` **sem** `prevSample` → sticky `IS PAUSED` (MSFS 2024 pós-ESC) bloqueava depart/sawAirborne enquanto `advanceFlightPhase` (display-only) ainda pintava climb; (2) Watch mid-air nunca re-latchava `originClearedForDepart` se o seed do Validate falhasse. Fix = passar `prevSample` (motion unfreeze) no Watch + CLI; mid-air latch quando LV ready + location OK ou já fora do hub.
