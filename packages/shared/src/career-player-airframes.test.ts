...
describe('career player airframes', () => {
  ...
  it('offers light GA and turboprop models as starter choices', () => {
    const starters = listStarterCareerPlayerAirframes();
    assert.ok(starters.some((row) => row.typeId === 'asobo-c172sp-cargo'));
    assert.ok(starters.some((row) => row.typeId === 'c208-caravan-cargo'));
    assert.ok(
      starters.every(
        (row) =>
          row.aircraftClassId === 'light_ga' ||
          row.aircraftClassId === 'light_turboprop',
      ),
    );
  });
  ...
  it('keeps concrete variants under their economic class', () => {
    ...
    assert.ok(ga.some((airframe) => airframe.simbriefIcao === 'C172'));
    assert.ok(ga.some((airframe) => airframe.simbriefIcao === 'AC11'));
    assert.ok(ga.some((airframe) => airframe.simbriefIcao === 'BE36'));
    assert.equal(ga.filter((a) => a.simbriefIcao === 'AC11').length, 1);
    assert.equal(listCareerPlayerAirframes('light_turboprop').length, 1);
  });
