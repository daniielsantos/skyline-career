...
26. Homologar wing tanks Twin Otter com vars corretas + writetest (só se quiser range completo).
27. Dual-client IPC — **só** se freeze Watch+inject ainda aparecer no 0.3.24+.
28. Next map slice: **Asia** (start small).
29. Re-run `npm run career-hubs -- missing` após rebuild shared.
