...
**Watch auto-start só no Dispatch (2026-09-25):** sintoma = Ready (OFP passed) → usuário sai pra Crew → decola → stepper fica READY / DISPATCHED; probe mostra AIRBORNE; copy “Take off when Watch is connected”. Causa = `eligible` do Watch exigia `tab === 'staging'` (só `in_flight` dispensava). Sem Watch = sem auto-depart. Fix = após Loaded vs Due (ou `in_flight`), auto-start **independente** da aba (Crew/Hangar/…).
**PHASE CLIMB + MISSION DISPATCHED (2026-09-25):** sintoma = footer `MSFS` + `PHASE CLIMB` / Aircraft Airborne, stepper preso em READY / DISPATCHED. Causa = (1) `evaluateMissionFlightTransition` chamava `isSimPlaybackFrozen(sample)` **sem** `prevSample` → sticky `IS PAUSED` (MSFS 2024 pós-ESC) bloqueava depart/sawAirborne enquanto `advanceFlightPhase` (display-only) ainda pintava climb; (2) Watch mid-air nunca re-latchava `originClearedForDepart` se o seed do Validate falhasse. Fix = passar `prevSample` (Absolute Time / motion unfreeze) no Watch + CLI; mid-air latch quando LV ready + location OK ou já fora do hub.
**Footer MSFS↔SIMBRIDGE flicker no open (2026-09-25):** sintoma = abrir app com Ready + DISPATCHED (ainda no chão) → chip do rodapé piscava MSFS/SIMBRIDGE uns segundos e depois estabilizava ON GROUND. Causa = auto-start Watch: effect remonta no meio do `postWatchStart` (`watch.running` / hydrate) → `cancelled && !in_flight` fazia `postWatchStop` na sessão que acabou de subir → pipe open/close a cada ~1s (log: start → tick → `client.close` → start; host IPC storm). Fix = só yield/stop quando Preflight ainda precisa do pipe (`stillNeedsHold`); com LV (ou `in_flight`) manter Watch e `setWatch` mesmo se o effect cancelou — mesmo padrão do reopen mid-cruise (af).
...
**Live cantos 90°+ / “não fixa” (2026-09-25):** sintoma = trilha azul com curvas impossíveis (reta origin→AC, zigue-zague >90°) — uns trechos “fixam”, outros o tip só desliza. Causa = crumb novo só com `MIN_MOVE` **0.35 nm** (early 0.12); entre updates o tip move in-place e o LineString liga vértices esparsos → corda pela curva. Uplink já é ~5s. Fix = limiar uniforme **0.05 nm** (ruído/GPS); cada POST que andou ≥ isso vira vértice. **Precisa redeploy world-api** (store in-memory na VPS).
**Live MAX_POINTS long haul (2026-09-25):** sintoma = 8k crumbs @ 5s ≈ 11 h / ~5 000 nm jet → long haul 6 000 nm podava o DEP. Fix = `FLIGHT_TRACK_MAX_POINTS` **15 000** (~21 h). Redeploy world-api.
**Resume prep @ origin (2026-09-25):** sintoma = MSFS crash/restart no DEP → En route **BACK AT DEPARTURE** sem inject (status `in_flight` bloqueava `/api/load-ofp`); reverter pra accepted/dispatched perderia lotes em trânsito / clock. Causa = apply OFP só `accepted|dispatched`; UI En route escondia toggle. Fix = `isResumePrepAtOrigin` (in_flight + ground + sawAirborne + nearOrigin + !nearDest); allow `in_flight` no load-ofp; En route mostra Airframe inject + Loaded vs Due (Due, não OFP dep) sem mudar status; Watch yield no inject e re-start após writes.
**Settle flash @ DEP open (2026-09-25):** sintoma = abrir En route BACK AT DEPARTURE → overlay **Settling flight…** um instante. Causa = poll Watch armava `settleOverlaySticky` com ground+sawAirborne+parkingBrake **sem** `destProximity.ok` (o effect otimista já exigia nearDest). Fix = `settleIntent` no poll também exige near dest para o braço parking-brake.
**Resume prep inject flicker + Accu-Sim dead probe (2026-09-25):** sintoma = resume inject Aerostar `PAYLOAD_NOT_APPLIED` 340 vs 921 (Preflight ok); toggle some no fail. Causa = mesmo `/api/load-ofp`, mas probe “dead holds” (pensado pro C408) lê classic stations — pós-restart Accu-Sim marca S3–S7 dead → cargoTarget 0 → só crew. Fix = **skip** esse probe em `a2a-lvars`/`lvar-bridge` (mesmo fill do Preflight); sticky UI em `failed`. Sem segundo inject.
**Resume prep layout shove + toggle blink (2026-09-25):** sintoma = (1) título `RESUME PREP`/`READY`/`BACK AT DEPARTURE` + status inline + `Checked …` empurravam o switch no inject; (2) toggle sumia e voltava ao terminar (Watch yield). Causa = `width:100%` no title dentro do flex head; headline mudava de comprimento; status no fluxo; latch só enquanto `loadOfp` status. Fix = título fixo `ON GROUND · RESUME PREP`; title ellipsis/`width:auto`; remove Checked do head En route; latch até airborne (Watch live) ou nearDest. **Status do inject** à esquerda do switch em `position:absolute` (fora do fluxo — aparece/some sem empurrar título nem botão).
**Resume prep layout + frozen LV (2026-09-25):** sintoma = (1) cards Fuel/Payload “encolhidos” sob ON GROUND; (2) números congelados (ex. Sim≈Due) pós-restart até inject. Causa = (1) `resumePrep` trocava `dispatch-enroute-live-tiles` por `preflight-load-grid` + head duplicado RESUME PREP READY; (2) Watch só amostrava LV no ramp com status **`dispatched`** — `in_flight` no chão nunca refresca. Fix = manter tiles En route + inject no live head; sample ramp também para `in_flight`.
**En route live fuel burn (2026-09-25):** sintoma = schematic de tanques congelado em voo (só snapshot do takeoff). Causa = sample LV exigia `onGround` (stations caras). Fix = `sampleLiveFuelLb` (batch fuel-only, sem stations) a cada ~10s em `in_flight` airborne; mantém payload/ready do pré-decolagem; soft marks OFP dep.
