...
- **World CI/CD (2026-09-16):** `World deploy` publica uma imagem GHCR única `linux/amd64,linux/arm64`. CI verde em `main` builda/publica `sha-<commit>` + `main`, mas não exige Pi ligado; Pi staging só promove por `workflow_dispatch target=staging`. GitHub Release consulta Actions pelo `workflow_run` + `head_sha`, aguarda até 30 min o build principal terminar com sucesso, confirma a visibilidade do manifest e retaga o mesmo digest como `v*` via `buildx imagetools` (sem segundo build). Exit 255 teve duas causas distintas: v0.3.69 esperava diretamente pelo registry antes do build terminar; v0.3.70 usava `awk ... exit` sob `pipefail`, fechava o pipe cedo e fazia o próprio `buildx inspect` falhar com SIGPIPE. O parser agora consome toda a saída. Depois promove à VPS após aprovação do Environment `production`; dispatch manual de production ainda builda a ref solicitada. Hosts fazem pull-only; `deploy-world.sh` para/remove qualquer worker legado, atualiza o single-writer API, valida contrato health e faz app rollback em falha. Produção cria `pg_dump` pré-deploy (14d); nunca remove/restaura volume automaticamente. `release-desktop` agora fetch tags, push commit antes da release e fixa `--target` no HEAD publicado para tag/installer/world usarem o mesmo código. Pi usa **OpenSSH normal sobre Tailscale** quando ligado (`tailscale set --ssh=false`). Setup: `deploy/README.md`.
