...
describe('career-economy seed', () => {
  it('creates Americas hubs across SA + Central America + Caribbean', () => {
    ...
    assert.equal(world.airports.length, 2231);
    ...
    const idn = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'ID',
    );
    const ph = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'PH',
    );
    const cn = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'CN',
    );
    const jp = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'JP',
    );
    const kr = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'KR',
    );
    const tw = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'TW',
    );
    const au = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'AU',
    );
    const nz = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'NZ',
    );
    const fj = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'FJ',
    );
    const pg = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'PG',
    );
    const nc = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'NC',
    );
    const pf = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'PF',
    );
    const pw = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'PW',
    );
    const ws = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'WS',
    );
    const tonga = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'TO',
    );
    const vu = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'VU',
    );
    const sb = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'SB',
    );
    const ck = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'CK',
    );
    const ki = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'KI',
    );
    const ru = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'RU',
    );
    const ng = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'NG',
    );
    ...
    const ci = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'CI',
    );
    const ke = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'KE',
    );
    const et = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'ET',
    );
    const za = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'ZA',
    );
    const tz = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'TZ',
    );
    const ao = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'AO',
    );
    const cm = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'CM',
    );
    const ug = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'UG',
    );
    const rw = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'RW',
    );
    const mz = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'MZ',
    );
    const na = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'NA',
    );
    const bw = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'BW',
    );
    const zm = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'ZM',
    );
    const zw = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'ZW',
    );
    const mw = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'MW',
    );
    const cd = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'CD',
    );
    const cg = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'CG',
    );
    const ga = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'GA',
    );
    const gq = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'GQ',
    );
    const cf = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'CF',
    );
    const td = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'TD',
    );
    const bi = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'BI',
    );
    const bj = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'BJ',
    );
    const tg = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'TG',
    );
    const bf = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'BF',
    );
    const ml = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'ML',
    );
    const ne = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'NE',
    );
    const gn = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'GN',
    );
    const sl = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'SL',
    );
    const lr = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'LR',
    );
    const gm = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'GM',
    );
    const gw = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'GW',
    );
    const cv = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'CV',
    );
    const st = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'ST',
    );
    const mr = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'MR',
    );
    const mg = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'MG',
    );
    const mu = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'MU',
    );
    const sc = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'SC',
    );
    const km = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'KM',
    );
    const ls = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'LS',
    );
    const sz = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'SZ',
    );
    const so = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'SO',
    );
    const dj = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'DJ',
    );
    const er = world.airports.filter(
      (a) => countryIdFromRegion(a.region) === 'ER',
    );
    ...
    assert.equal(dz.length, 8);
    assert.equal(tn.length, 6);
    assert.equal(eg.length, 10);
    assert.equal(il.length, 4);
    assert.equal(sa.length, 16);
    assert.equal(ae.length, 9);
    assert.equal(qa.length, 2);
    assert.equal(bh.length, 1);
    assert.equal(kw.length, 1);
    assert.equal(om.length, 6);
    assert.equal(iq.length, 8);
    assert.equal(ir.length, 12);
    assert.equal(jo.length, 3);
    assert.equal(lb.length, 1);
    assert.equal(sy.length, 4);
    ...
    assert.ok(world.airports.some((airport) => airport.icao === 'WIII'));
    assert.ok(world.airports.some((airport) => airport.icao === 'WARR'));
    assert.ok(world.airports.some((airport) => airport.icao === 'WIMM'));
    assert.ok(world.airports.some((airport) => airport.icao === 'WADD'));
    assert.ok(world.airports.some((airport) => airport.icao === 'WALL'));
    assert.ok(world.airports.some((airport) => airport.icao === 'WAAA'));
    assert.ok(world.airports.some((airport) => airport.icao === 'WBKK'));
    assert.ok(world.airports.some((airport) => airport.icao === 'WBGG'));
    assert.ok(world.airports.some((airport) => airport.icao === 'RPLL'));
    assert.ok(world.airports.some((airport) => airport.icao === 'RPVM'));
    assert.ok(world.airports.some((airport) => airport.icao === 'RPMY'));
    assert.equal(
      world.airports.find((a) => a.icao === 'WIII')?.hubTier,
      'major',
    );
    assert.equal(
    ...
    assert.equal(
      world.airports.some((airport) => airport.icao === 'RPML'),
      false,
    );
    assert.ok(world.airports.some((airport) => airport.icao === 'ZBAA'));
    assert.ok(world.airports.some((airport) => airport.icao === 'ZSPD'));
    assert.ok(world.airports.some((airport) => airport.icao === 'ZSSS'));
    assert.ok(world.airports.some((airport) => airport.icao === 'ZGGG'));
    assert.ok(world.airports.some((airport) => airport.icao === 'ZGSZ'));
    assert.ok(world.airports.some((airport) => airport.icao === 'ZUUU'));
    assert.ok(world.airports.some((airport) => airport.icao === 'RJAA'));
    assert.ok(world.airports.some((airport) => airport.icao === 'RJTT'));
    assert.ok(world.airports.some((airport) => airport.icao === 'RJBB'));
    assert.ok(world.airports.some((airport) => airport.icao === 'RJFF'));
    assert.ok(world.airports.some((airport) => airport.icao === 'RJCC'));
    assert.ok(world.airports.some((airport) => airport.icao === 'RKSI'));
    assert.ok(world.airports.some((airport) => airport.icao === 'RKPK'));
    assert.ok(world.airports.some((airport) => airport.icao === 'RKPC'));
    assert.ok(world.airports.some((airport) => airport.icao === 'RCTP'));
    assert.ok(world.airports.some((airport) => airport.icao === 'RCSS'));
    assert.ok(world.airports.some((airport) => airport.icao === 'RCKH'));
    assert.ok(world.airports.some((airport) => airport.icao === 'YSSY'));
    assert.ok(world.airports.some((airport) => airport.icao === 'YPDN'));
    assert.ok(world.airports.some((airport) => airport.icao === 'YMHB'));
    assert.ok(world.airports.some((airport) => airport.icao === 'WBSB'));
    assert.ok(world.airports.some((airport) => airport.icao === 'NZWN'));
    assert.ok(world.airports.some((airport) => airport.icao === 'RCMQ'));
    assert.ok(world.airports.some((airport) => airport.icao === 'ZSQD'));
    assert.ok(world.airports.some((airport) => airport.icao === 'ZSHC'));
    assert.ok(world.airports.some((airport) => airport.icao === 'ZYTX'));
    assert.ok(world.airports.some((airport) => airport.icao === 'ZJHK'));
    assert.ok(world.airports.some((airport) => airport.icao === 'UUEE'));
    assert.ok(world.airports.some((airport) => airport.icao === 'ULLI'));
    assert.ok(world.airports.some((airport) => airport.icao === 'UNNT'));
    assert.ok(world.airports.some((airport) => airport.icao === 'UNKL'));
    assert.ok(world.airports.some((airport) => airport.icao === 'ULMM'));
    assert.ok(world.airports.some((airport) => airport.icao === 'UMKK'));
    assert.ok(world.airports.some((airport) => airport.icao === 'URSS'));
    assert.ok(world.airports.some((airport) => airport.icao === 'UWKD'));
    assert.ok(world.airports.some((airport) => airport.icao === 'UHSS'));
    assert.ok(world.airports.some((airport) => airport.icao === 'UHWW'));
    assert.ok(world.airports.some((airport) => airport.icao === 'URRP'));
    assert.equal(
      world.airports.some((airport) => airport.icao === 'URRR'),
      false,
      'URRR is closed; seed uses URRP Platov',
    );
    assert.ok(world.airports.some((airport) => airport.icao === 'DNMM'));
    assert.ok(world.airports.some((airport) => airport.icao === 'DNAA'));
    assert.ok(world.airports.some((airport) => airport.icao === 'DNPO'));
    assert.ok(world.airports.some((airport) => airport.icao === 'DGAA'));
    assert.ok(world.airports.some((airport) => airport.icao === 'GOOY'));
    assert.ok(world.airports.some((airport) => airport.icao === 'DIAP'));
    assert.ok(world.airports.some((airport) => airport.icao === 'HKJK'));
    assert.ok(world.airports.some((airport) => airport.icao === 'HKMO'));
    assert.ok(world.airports.some((airport) => airport.icao === 'HAAB'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FAOR'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FACT'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FALE'));
    assert.ok(world.airports.some((airport) => airport.icao === 'HTDA'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FNLU'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FKKD'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FKYS'));
    assert.equal(
      world.airports.find((a) => a.icao === 'DNMM')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'FAOR')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'HKJK')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'GOBD'),
      false,
      'Dakar stock ident is GOOY, not GOBD',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'FADN'),
      false,
      'Durban is FALE King Shaka, not closed FADN',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'FNUB'),
      false,
      'Luanda stock ident is FNLU, not FNUB',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'HKNW'),
      false,
      'Nairobi cargo major is HKJK, not HKNW Wilson',
    );
    assert.ok(world.airports.some((airport) => airport.icao === 'DNKN'));
    assert.ok(world.airports.some((airport) => airport.icao === 'DGSI'));
    assert.ok(world.airports.some((airport) => airport.icao === 'HUEN'));
    assert.ok(world.airports.some((airport) => airport.icao === 'HRYR'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FQMA'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FQBR'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FYWH'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FYWB'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FBSK'));
    assert.equal(
      world.airports.find((a) => a.icao === 'HUEN')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'FYWH')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'FYWE'),
      false,
      'Windhoek cargo major is FYWH, not FYWE Eros',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'FBMN'),
      false,
      'Gaborone cargo major is FBSK, not FBMN Maun',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'FQBE'),
      false,
      'Beira stock ident is FQBR, not FQBE',
    );
    assert.ok(world.airports.some((airport) => airport.icao === 'FLKK'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FVHA'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FVBU'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FWKI'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FWCL'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FZAA'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FZQA'));
    assert.ok(world.airports.some((airport) => airport.icao === 'GOTT'));
    assert.equal(
      world.airports.find((a) => a.icao === 'FLKK')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'FZAA')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'FLLS'),
      false,
      'Lusaka stock ident is FLKK, not FLLS',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'FVRG'),
      false,
      'Harare stock ident is FVHA, not FVRG',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'FVJN'),
      false,
      'Bulawayo stock ident is FVBU, not FVJN',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'FZAB'),
      false,
      "Kinshasa cargo major is FZAA, not FZAB N'dolo",
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'GOTB'),
      false,
      'Tambacounda is GOTT, not GOTB Bakel',
    );
    assert.ok(world.airports.some((airport) => airport.icao === 'FCBB'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FCPP'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FOOL'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FOOG'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FGSL'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FEFF'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FTTJ'));
    assert.ok(world.airports.some((airport) => airport.icao === 'HBBA'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FZIC'));
    assert.equal(
      world.airports.find((a) => a.icao === 'FCBB')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'FOOL')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'FZIA'),
      false,
      'Kisangani cargo is FZIC Bangoka, not FZIA Simisini',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'OCS'),
      false,
      'Malabo is FGSL, not OCS Corisco',
    );
    assert.ok(world.airports.some((airport) => airport.icao === 'DBBB'));
    assert.ok(world.airports.some((airport) => airport.icao === 'DXXX'));
    assert.ok(world.airports.some((airport) => airport.icao === 'DFFD'));
    assert.ok(world.airports.some((airport) => airport.icao === 'GABS'));
    assert.ok(world.airports.some((airport) => airport.icao === 'DRRN'));
    assert.ok(world.airports.some((airport) => airport.icao === 'GUCY'));
    assert.ok(world.airports.some((airport) => airport.icao === 'GFLL'));
    assert.ok(world.airports.some((airport) => airport.icao === 'GLRB'));
    assert.ok(world.airports.some((airport) => airport.icao === 'GBYD'));
    assert.ok(world.airports.some((airport) => airport.icao === 'GGOV'));
    assert.ok(world.airports.some((airport) => airport.icao === 'GVAC'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FPST'));
    assert.ok(world.airports.some((airport) => airport.icao === 'GQNO'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FMMI'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FMMT'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FIMP'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FSIA'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FMCH'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FXMM'));
    assert.ok(world.airports.some((airport) => airport.icao === 'FDSK'));
    assert.ok(world.airports.some((airport) => airport.icao === 'HCMM'));
    assert.ok(world.airports.some((airport) => airport.icao === 'HDAM'));
    assert.ok(world.airports.some((airport) => airport.icao === 'HHAS'));
    assert.ok(world.airports.some((airport) => airport.icao === 'HJJJ'));
    assert.ok(world.airports.some((airport) => airport.icao === 'LPMA'));
    assert.ok(world.airports.some((airport) => airport.icao === 'LPPD'));
    assert.ok(world.airports.some((airport) => airport.icao === 'GCLP'));
    assert.equal(
      world.airports.find((a) => a.icao === 'HJJJ')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'HSSJ'),
      false,
      'Juba cargo major is HJJJ, not legacy HSSJ',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'LPMA')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'LPPD')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'GCLP')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'LPPS'),
      false,
      'Madeira cargo major is LPMA, not LPPS Porto Santo',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'LPLA'),
      false,
      'Azores cargo major is LPPD, not LPLA Lajes',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'GCTS'),
      false,
      'Canaries cargo major is GCLP, not GCTS Tenerife',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'FMMI')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'GQNN'),
      false,
      'Nouakchott cargo major is GQNO, not closed GQNN',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'FIMR'),
      false,
      'Mauritius cargo major is FIMP, not FIMR Rodrigues',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'FSPP'),
      false,
      'Seychelles cargo major is FSIA, not FSPP Praslin',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'FDMS'),
      false,
      'Eswatini cargo major is FDSK, not FDMS Matsapha',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'GVAC')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'DXNG'),
      false,
      'Lomé stock ident is DXXX, not DXNG Niamtougou',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'GAGO'),
      false,
      'Bamako cargo major is GABS, not GAGO Gao',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'GLMR'),
      false,
      'Monrovia cargo major is GLRB, not GLMR Spriggs Payne',
    );
    assert.ok(world.airports.some((airport) => airport.icao === 'YSCB'));
    assert.ok(world.airports.some((airport) => airport.icao === 'YMML'));
    assert.ok(world.airports.some((airport) => airport.icao === 'YPAD'));
    assert.ok(world.airports.some((airport) => airport.icao === 'YBBN'));
    assert.ok(world.airports.some((airport) => airport.icao === 'YBCS'));
    assert.ok(world.airports.some((airport) => airport.icao === 'YPPH'));
    assert.ok(world.airports.some((airport) => airport.icao === 'NZAA'));
    assert.ok(world.airports.some((airport) => airport.icao === 'NZCH'));
    assert.ok(world.airports.some((airport) => airport.icao === 'ZLXY'));
    assert.ok(world.airports.some((airport) => airport.icao === 'ZPPP'));
    assert.ok(world.airports.some((airport) => airport.icao === 'ZYTL'));
    assert.ok(world.airports.some((airport) => airport.icao === 'ZUCK'));
    assert.ok(world.airports.some((airport) => airport.icao === 'ZHHH'));
    assert.ok(world.airports.some((airport) => airport.icao === 'ZSAM'));
    assert.ok(world.airports.some((airport) => airport.icao === 'PHNL'));
    assert.ok(world.airports.some((airport) => airport.icao === 'NFFN'));
    assert.ok(world.airports.some((airport) => airport.icao === 'AYPY'));
    assert.ok(world.airports.some((airport) => airport.icao === 'NWWW'));
    assert.ok(world.airports.some((airport) => airport.icao === 'PGUM'));
    assert.ok(world.airports.some((airport) => airport.icao === 'NTAA'));
    assert.ok(world.airports.some((airport) => airport.icao === 'PTRO'));
    assert.ok(world.airports.some((airport) => airport.icao === 'ANG'));
    assert.ok(world.airports.some((airport) => airport.icao === 'NSTU'));
    assert.ok(world.airports.some((airport) => airport.icao === 'NSFA'));
    assert.ok(world.airports.some((airport) => airport.icao === 'NFTF'));
    assert.ok(world.airports.some((airport) => airport.icao === 'NVVV'));
    assert.ok(world.airports.some((airport) => airport.icao === 'AGGH'));
    assert.ok(world.airports.some((airport) => airport.icao === 'NCRG'));
    assert.ok(world.airports.some((airport) => airport.icao === 'NGTA'));
    assert.ok(world.airports.some((airport) => airport.icao === 'PGSN'));
    assert.ok(world.airports.some((airport) => airport.icao === 'PLCH'));
    assert.ok(world.airports.some((airport) => airport.icao === 'NFTV'));
    assert.ok(world.airports.some((airport) => airport.icao === 'NVSS'));
    assert.ok(world.airports.some((airport) => airport.icao === 'AGGM'));
    assert.ok(world.airports.some((airport) => airport.icao === 'NCAI'));
    assert.ok(world.airports.some((airport) => airport.icao === 'NTTB'));
    assert.ok(world.airports.some((airport) => airport.icao === 'NSAU'));
    assert.equal(
      world.airports.find((a) => a.icao === 'ZBAA')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'ZSPD')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'RJAA')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'RKSI')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'RCTP')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'YSSY')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'YPDN')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'YMML')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'NZAA')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'PHNL')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'NFFN')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'PGUM')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'NTAA')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'NVVV')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'AGGH')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'PGSN')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'PLCH')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'NFTV')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'NVSS')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'AGGM')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'NCAI')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'NTTB')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'NSAU')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'ANG')?.hubTier,
      'major',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'ZBAD'),
      false,
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'RKSS'),
      false,
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'RCTP'),
      true,
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'YSBK'),
      false,
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'YMEN'),
      false,
    );
    assert.equal(
      world.airports.find((a) => a.icao === 'NZWN')?.hubTier,
      'regional',
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'ZLSN'),
      false,
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'ZUTF'),
      false,
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'ZBTJ'),
      false,
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'PGUA'),
      false,
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'NWWM'),
      false,
    );
    assert.equal(
      world.airports.some((airport) => airport.icao === 'PGUA'),
    ...
    assert.equal(
      world.airports.some((airport) => airport.icao === 'NGTU'),
      false,
    );
    assert.equal(
...
describe('migrateEconomyWorld / ensureEconomyCaughtUp', () => {
  it('adds missing BR-N / BR-CO hubs to a truncated legacy airport list', () => {
    ...
    assert.equal(migrated.airports.length, 2231);
    ...
    assert.ok(migrated.airports.some((a) => a.icao === 'WIII'));
    assert.ok(migrated.airports.some((a) => a.icao === 'WIMM'));
    assert.ok(migrated.airports.some((a) => a.icao === 'WBKK'));
    assert.ok(migrated.airports.some((a) => a.icao === 'RPLL'));
    assert.ok(migrated.airports.some((a) => a.icao === 'RPVM'));
    assert.ok(migrated.airports.some((a) => a.icao === 'ZBAA'));
    assert.ok(migrated.airports.some((a) => a.icao === 'ZSPD'));
    assert.ok(migrated.airports.some((a) => a.icao === 'RJAA'));
    assert.ok(migrated.airports.some((a) => a.icao === 'RKSI'));
    assert.ok(migrated.airports.some((a) => a.icao === 'RCTP'));
    assert.ok(migrated.airports.some((a) => a.icao === 'YSSY'));
    assert.ok(migrated.airports.some((a) => a.icao === 'NZAA'));
    assert.ok(migrated.airports.some((a) => a.icao === 'ZLXY'));
    assert.ok(migrated.airports.some((a) => a.icao === 'ZYTL'));
    assert.ok(migrated.airports.some((a) => a.icao === 'PHNL'));
    assert.ok(migrated.airports.some((a) => a.icao === 'NFFN'));
    assert.ok(migrated.airports.some((a) => a.icao === 'AYPY'));
    assert.ok(migrated.airports.some((a) => a.icao === 'NWWW'));
    assert.ok(migrated.airports.some((a) => a.icao === 'PGUM'));
    assert.ok(migrated.airports.some((a) => a.icao === 'NTAA'));
    assert.ok(migrated.airports.some((a) => a.icao === 'NSFA'));
    assert.ok(migrated.airports.some((a) => a.icao === 'NVVV'));
    assert.ok(migrated.airports.some((a) => a.icao === 'AGGH'));
    assert.ok(migrated.airports.some((a) => a.icao === 'NCRG'));
    assert.ok(migrated.airports.some((a) => a.icao === 'NGTA'));
    assert.ok(migrated.airports.some((a) => a.icao === 'PGSN'));
    assert.ok(migrated.airports.some((a) => a.icao === 'PLCH'));
    assert.ok(migrated.airports.some((a) => a.icao === 'NFTV'));
    assert.ok(migrated.airports.some((a) => a.icao === 'NVSS'));
    assert.ok(migrated.airports.some((a) => a.icao === 'AGGM'));
    assert.ok(migrated.airports.some((a) => a.icao === 'NCAI'));
    assert.ok(migrated.airports.some((a) => a.icao === 'NTTB'));
    assert.ok(migrated.airports.some((a) => a.icao === 'NSAU'));
    assert.ok(migrated.airports.some((a) => a.icao === 'ANG'));
    assert.ok(migrated.airports.some((a) => a.icao === 'YPDN'));
    assert.ok(migrated.airports.some((a) => a.icao === 'WBSB'));
    assert.ok(migrated.airports.some((a) => a.icao === 'UUEE'));
    assert.ok(migrated.airports.some((a) => a.icao === 'DNMM'));
    assert.ok(migrated.airports.some((a) => a.icao === 'FAOR'));
    assert.ok(migrated.airports.some((a) => a.icao === 'HKJK'));
    assert.ok(migrated.airports.some((a) => a.icao === 'HUEN'));
    assert.ok(migrated.airports.some((a) => a.icao === 'FQMA'));
    assert.ok(migrated.airports.some((a) => a.icao === 'FYWH'));
