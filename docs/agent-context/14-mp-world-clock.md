...
- **World CI/CD (2026-09-16):** `World deploy` publica uma imagem GHCR única `linux/amd64,linux/arm64`. CI verde em `main` promove digest imutável ao Pi via Tailscale; GitHub Release promove a mesma revisão à VPS após aprovação do Environment `production`. Hosts fazem pull-only; `deploy-world.sh` para worker, atualiza API, valida contrato health, atualiza worker e faz app rollback em falha. Produção cria `pg_dump` pré-deploy (14d); nunca remove/restaura volume automaticamente. `release-desktop` agora fetch tags, push commit antes da release e fixa `--target` no HEAD publicado para tag/installer/world usarem o mesmo código. Pi usa **OpenSSH normal sobre Tailscale** (`tailscale set --ssh=false`); Tailscale SSH intercepta :22 e rejeita a deploy key. Primeiro pull/extract multiarch no microSD excedeu 15 min sem tocar containers; staging timeout agora 45 min (pulls seguintes reutilizam layers). Setup: `deploy/README.md`.
- **World image slim (2026-09-16):** `Dockerfile.world` agora é multi-stage. A fase builder mantém o install/build do monorepo; a imagem publicada leva só Node slim, runtime lock dedicado (`pg` + `tsx`), `shared/runtime` compilados, fontes estritamente necessárias de API/agent, worker e seed career. Electron, Vite, Catalog API e toolchain não entram na camada final. Validação efetiva de tamanho/boot fica para o primeiro build/deploy GHCR.
