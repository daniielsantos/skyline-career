...
**Doc 2026-09-22 (d):** Crew Roster **Live** — Watch uploads position (~15s) to in-memory `/api/va/flight-track`; members GET hint `live`; map OD dashed + flown trail + progress. **(e)** sample + chip: phase / onGround / altFt / gsKt (Watch; refresh even sem mover trilha). **(f)** arma upload ao **ligar motores** (accepted/dispatched), não só `in_flight`. **(g)** Live pane sem flicker de Loading; OD enquanto espera motores.
